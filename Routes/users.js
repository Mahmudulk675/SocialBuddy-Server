...
// router.get("/", (req, res) => {
//   res.send("Homepage user Loaded");
// });

// update user
router.put("/:id", async (req, res) => {
  if (req.body.userId === req.params.id || req.body.inAdmin) {
...
router.delete("/:id", async (req, res) => {
  if (req.body.userId === req.params.id || req.body.inAdmin) {
    ...
    return res.status(403).json("Delete Permission Denied");
  }
});
  ...
  if (req.body.userId !== req.params.id) {
    try {
      ...
      if (user.followers.includes(req.body.userId)) {
        ...
        res.status(500).json("The user is unfollowed already!");
      }
    } catch (err) {
      res.status(500).json(err);
    }
